...
(function (global) {
  function dependsOptionsHtml(items, course, selected, excludeItem) {
    ...
    return (
      ...
      matches
        .map(
          (it) =>
            `<option value="${it.title}" ${selected === it.title ? 'selected' : ''}>${it.title}</option>`
        )
        .join('')
    );
  }
  ...
  function formHtml(items, existing) {
    ...
    return `<h3>${existing ? 'Edit Assignment' : 'Add Assignment'}</h3>
      <input id="tp-title" placeholder="Title" value="${e.title}">
      <input id="tp-course" placeholder="Course (e.g. NT)" value="${e.course || ''}">
      ...
      <input id="tp-unitlabel" placeholder="Unit (pages, chapters, problems...)" value="${e.unit || ''}">
      <textarea id="tp-notes" placeholder="Notes (optional)">${e.notes || ''}</textarea>
      <textarea id="tp-subtasks" placeholder="Subtasks, one per line (optional)">${(e.subtasks || []).map((s) => s.text).join('\n')}</textarea>
      <label>Repeat:</label>
